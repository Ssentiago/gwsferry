...
func ConnectAndAuth(email, token string) (*client.Client, error) {
	...
	// go-imap v1: Client.Timeout управляет TCP deadline для каждой операции.
	// Без этого UidFetch на большом ящике зависает навсегда.
	...
	// После auth: сбрасываем общий deadline, но c.Timeout продолжает
	// управлять deadline для каждой операции через execute().
